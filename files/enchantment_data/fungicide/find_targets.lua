...
local hit = PredictProjectileTarget(entity, function(hitinfo)
	if(valid_herds[HerdIdToString(EntityGetHerdID( hitinfo.target ))])then
		...
		if(not GameHasEntityFlagRun(child, "fungicide_damage_boosted"))then
			adjust_all_entity_damage(entity, function(damage)
				return damage * 1.5
			end)
			GameAddEntityFlagRun(child, "fungicide_damage_boosted")
		end
	end
end)

if(not hit)then
	if(GameHasEntityFlagRun(child, "fungicide_damage_boosted"))then
		adjust_all_entity_damage(entity, function(damage)
			return damage / 1.5
		end)
